...
const STATE_LABELS = {
  ...
  SHIPPING: { label: "ממתין למשלוח", color: "bg-blue-100 text-blue-800" },
...
export default function SPOrderDetailsModal({ order, open, onClose, onRefresh }) {
  ...
  const handleAction = async (action) => {
    setLoading(action);
    try {
      const { data } = await updateSuperPharmOrder({ action, order_id: order.mirakl_order_id });
      if (data.success) {
  ...
  const handleCreateInvoice = () => {
    // Navigate or trigger Linet invoice for this order
    toast.info("יצירת חשבונית בלינט - בקרוב");
  };

  return (
    <>
      <Dialog open={open && !showShipDialog} onOpenChange={onClose}>
