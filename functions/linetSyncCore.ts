  ...
  if (existing.length > 0) {
    ...
    await base44.asServiceRole.entities.UndeliveredOrderTask.create({
      ...
      activity_log: JSON.stringify([
        { action: 'נוצר אוטומטית מסנכרון לינט', user: 'system', timestamp: new Date().toISOString() },
      ]),
    });
    return 'created';
  }
...
export async function executeLinetSync(base44, body = {}) {
  ...
  try {
    ...
    try {
      if (syncLog) {
        await base44.asServiceRole.entities.SyncLog.update(syncLog.id, { run_finished_at: new Date().toISOString(), status: 'FAILED', error_message: errorMessage });
      }
      const metadataList = await base44.asServiceRole.entities.SyncMetadata.filter({ sync_key: SYNC_KEY });
      if (metadataList[0]) {
