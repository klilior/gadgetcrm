...
function normalizeE164(value) {
  let digits = clean(value, 32).replace(/\D/g, '');
...
function displayText(body) {
  ...
  return labels[clean(body.message_type, 40)] || '[הודעת WhatsApp]';
}

async function validSignature(rawBody, timestamp, supplied, secret) {
  if (!/^\d{10,13}$/.test(timestamp)) return false;
  const epoch = timestamp.length === 13
    ? Math.floor(Number(timestamp) / 1000)
    : Number(timestamp);
  if (!Number.isFinite(epoch)) return false;
  if (Math.abs(Math.floor(Date.now() / 1000) - epoch) > MAX_CLOCK_SKEW_SECONDS) {
    return false;
  }
  ...
  return crypto.subtle.verify(
    'HMAC', key, bytes, encoder.encode(timestamp + '.' + rawBody)
  );
}
...
Deno.serve(async (req) => {
  ...
  if (!(await validSignature(rawBody, timestamp, signature, secret))) {
  ...
  try {
    const existing = await entities.WhatsAppCloudMessage.filter({
    ...
    const phone = localPhone(fromE164);
    let clients = await entities.Client.filter({ phone });
    ...
    if (!customer) {
      customer = await entities.Client.create({
        full_name: clean(body.profile_name, 200) || phone,
        phone,
    ...
    const content = displayText(body);
    const activities = await entities.Activity.filter({ thread_id: messageId });
