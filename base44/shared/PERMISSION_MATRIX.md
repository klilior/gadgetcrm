...
| PredefinedResponse | unsupported business-role list in user_condition | omitted/open | inherits legacy write | inherits legacy write | True admin intended | Manager/agent labels are not auth roles | Open create and unreliable role checks | HIGH | Authenticated read; admin-managed writes |
...
Product; LinetProductMap; LineProductDefinition; LinetCategoryTranslation; CarrierPolicy; CarrierProductMapping; Product monitoring records; PriceSnapshot; PriceRecommendation; PriceAlert; ProductsMonitor; SyncMetadata; SyncLog; WebhookLog; ClientErrorLog; PriceMonitorLog; GetPackageShipment; LinetUsersMap; PredefinedResponse-related content; Notification templates.
...
3. HIGH: Task, ShiftRequest, WeeklySchedule and PredefinedResponse use business roles as if they were Base44 auth roles.
